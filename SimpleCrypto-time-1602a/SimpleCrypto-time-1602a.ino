...
uint8_t clock_symb[8] = {0x0,0xe,0x15,0x17,0x11,0xe,0x0};
...
int wifiConnect(char ssid[], char password[]) {
  ...
  */
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    display.display();
    */
    displayMessage("WiFi connected:" + (String)(ssid));
  } else  // Failed to connect
  {
    ...
    display.display();
    */
    displayMessage("Failed to connect to"+(String)ssid);
  };
...
void setup() {
  ...
  // create clock char - usage: lcd.printByte(2); 
  lcd.createChar(2, clock_symb); 
...
void displayHolding(int index, String addString = "") {
  ...
  lcd.print((String)response.symbol+" ");
  ...
  lcd.print(formatCurrency(price, holdings[index].curSymb));
  
  lcd.setCursor(0, 1);      // At column=0, row=1  
  //display.setFont(ArialMT_Plain_16);
  //display.setTextAlignment(TEXT_ALIGN_CENTER);
  ...
  { // show date or time
    ...
    lcd.printByte(2); // show clock symbol
    lcd.print(" "+(String)addString + " " + String(response.percent_change_24h) + "%");
  }
  else
  {
    //display.drawString(64, 48, "24h: " + String(response.percent_change_24h) + "%");
    lcd.print( "24h: " + String(response.percent_change_24h) + "%");
  }
  //display.display();
}
